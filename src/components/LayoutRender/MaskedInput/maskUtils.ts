...
export const MASK_TYPES = ["cpf", "cnpj", "telefone", "telefone_simples", "celular", "celular_simples", "telefone_ddd", "celular_ddd", "rg", "cep", "data", "card_date", "hora", "data_hora", "placa", "cartao_credito", "pix", "usd", "brl", "porcentagem", "numero_inteiro", "numero_decimal"];
...
export const applyCardDateMask = (value: string): string => {
  const cleanValue = removeMask(value);
  if (cleanValue.length === 0) return "";
...
export const applyMask = (value: string, maskType: MaskType): string => {
  switch (maskType) {
    ...
    case "card_date":
      return applyCardDateMask(value);
    case "data_hora":
