...
import MaskedInput, { MaskType } from "../MaskedInput";
//import { CustomDatePicker } from "../CustomDatePicker";
import ComboCheckbox from "../ComboCheckbox";
import { Textarea } from "@/components/ui/textarea";
import { dev_log } from "@/lib/utils";
import { CustomDatePicker } from "@/lib/sbs-form-components/src/components/CustomDatePicker";
...
function GenericField({
  ...
  }, [value, field.apiConfig?.type, field.campoApi, loadingFields]);
        ...
        (!field.qtdRespostas || field.qtdRespostas <= 1) && (
          ...
          >
            <MaskedInput
              ...
              onBlur={async () => {
                if (field.campoApi === "cpf" && value && value.length >= 11) {
                  try {
                    setApiStatus('loading');
                    const res: any = await execApi({
                      url: `api/crm/client/find/${value.replace(/\D/g, "")}`,
                      method: "GET",
                      isCrmApi: true,
                      needLogout: true,
                      data: {},
                    ...
                    if (res.data) {
                      ...
                      if (result.sucesso && result.data) {
                        // Preenche automaticamente outros campos
                        if (result.data.nome) (onFieldUpdate || handleFieldUpdate)("nome", result.data.nome);
                        if (result.data.email) (onFieldUpdate || handleFieldUpdate)("email", result.data.email);
                        if (result.data.telefone) (onFieldUpdate || handleFieldUpdate)("telefone", result.data.telefone);
                        setApiStatus('success');
        ...
        (!field.qtdRespostas || field.qtdRespostas <= 1) && (
          ...
          >
            <Input
              ...
              id={field.campoApi}
              maxLength={field.tamanho ? parseInt(field.tamanho) : 999}
              value={field.campoApi === "placa" ? value.toUpperCase() : value}
              onChange={(e) => {
                let newValue = e.target.value;
                if(field.campoApi === "placa") newValue = newValue.toUpperCase();
                setValue(newValue);
                onValueChange?.(newValue);
              }}
              onBlur={async () => {
                if (field.campoApi === "placa" && value && value.length >= 7) {
                  try {
                    setApiStatus('loading');
                    const res: any = await execApi({
                      url: `api/crm/vehicle/find/${value}`,
                      method: "GET",
                      isCrmApi: true,
                      needLogout: true,
                      data: {},
...
export default memo(GenericField, (prevProps, nextProps) => {
  ...
  return (
    ...
    prevProps.field.apiConfig === nextProps.field.apiConfig &&
