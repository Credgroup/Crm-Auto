...
import type { FieldType, TpOptions } from "../../core/types";
...
import { useFieldApi } from "../../core/useFieldApi";
...
function GenericField({
  ...
  const [apiCallTimeout, setApiCallTimeout] = useState<any | null>(null);
  ...
  const isDateChangeFromUserRef = useRef(false);
  const lastDateValueRef = useRef<string | undefined>(field.conteudo);
  const dateRef = useRef<Date | undefined>(getValue(field));
  ...
  // Função wrapper para setDate que marca quando a mudança vem do usuário
  const setDateFromUser = useCallback((newDate: Date | undefined) => {
    isDateChangeFromUserRef.current = true;
  ...
  useEffect(() => {
    // Para campos de data, sincroniza o estado date apenas quando vem do parent
    if (field.type === "date") {
      const fieldDateString = field.conteudo;
      // Só sincroniza se o valor mudou e não veio do usuário
      if (fieldDateString !== lastDateValueRef.current && !isDateChangeFromUserRef.current) {
        const fieldDate = fieldDateString ? parseISO(fieldDateString) : undefined;
        // Compara timestamps usando a ref para evitar atualizações desnecessárias
        const currentDate = dateRef.current;
        if (fieldDate && (!currentDate || fieldDate.getTime() !== currentDate.getTime())) {
          isUpdatingFromParentRef.current = true;
          dateRef.current = fieldDate;
          setDate(fieldDate);
          lastDateValueRef.current = fieldDateString;
          isUpdatingFromParentRef.current = false;
        } else if (!fieldDate && currentDate) {
          // Se o campo foi limpo, limpa o date também
          isUpdatingFromParentRef.current = true;
      ...
      } else if (fieldDateString !== lastDateValueRef.current) {
        // Atualiza a ref mesmo quando veio do usuário para manter sincronizado
        lastDateValueRef.current = fieldDateString;
      }
    } else {
  ...
  }, [field.conteudo, field.mask, field.type]);
  ...
  // Atualiza o campo quando date muda vindo do usuário
  useEffect(() => {
    // Só chama onValueChange se a mudança veio do usuário (CustomDatePicker)
    if (date && isDateChangeFromUserRef.current && !isUpdatingFromParentRef.current) {
      const dateFormated = format(date, "yyyy-MM-dd");
      // Atualiza a ref para evitar re-sincronização
      lastDateValueRef.current = dateFormated;
      onValueChange?.(dateFormated);
      // Reset flag após chamar onValueChange para evitar loops
      isDateChangeFromUserRef.current = false;
    } else if (!date && isDateChangeFromUserRef.current && !isUpdatingFromParentRef.current) {
      // Se o date foi limpo pelo usuário, também atualiza
      lastDateValueRef.current = undefined;
      onValueChange?.("");
      isDateChangeFromUserRef.current = false;
    }
  }, [date, onValueChange]);
    ...
    if (field.type === "calculado" && field.calculo) {
      ...
      try {
        ...
        if (
          ...
          const resultado = parser.evaluate(expressao, scope);
  ...
  return (
    <div className="flex flex-col items-start gap-y-2 justify-start w-full">
      ...
      {field.type === "date" &&
        (!field.qtdRespostas || field.qtdRespostas <= 1) && (
        <CustomDatePicker field={field} date={date} setDate={setDateFromUser} />
      )}
