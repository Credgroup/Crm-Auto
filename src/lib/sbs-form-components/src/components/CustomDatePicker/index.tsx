...
export function CustomDatePicker({ field, date, setDate }: any) {
  ...
  const [daySelected, setDaySelected] = useState<string>(date ? String(date.getDate()) : "");
  const [monthSelected, setMonthSelected] = useState<string>(date ? String(date.getMonth()) : "");
  const [yearSelected, setYearSelected] = useState<string>(date ? String(date.getFullYear()) : "");

  useEffect(() => {
    if (isOpen) {
      if (date) {
        setDaySelected(String(date.getDate()));
        setMonthSelected(String(date.getMonth()));
        setYearSelected(String(date.getFullYear()));
      } else {
        setDaySelected("");
        setMonthSelected("");
        setYearSelected("");
  ...
  const { minDate, maxDate } = useMemo(() => {
    ...
    const isMinTodayMax45 = field?.dateConfig === "minTodayMax45";
    const isMinToday = field?.dateConfig === "minToday" || isMinTodayMax45;
    const isMaxToday = field?.dateConfig === "maxToday";

    const min = isMinToday ? today : new Date(1900, 0, 1);
    
    let max = new Date(2100, 11, 31);
    if (isMaxToday) max = today;
    if (isMinTodayMax45) {
      max = new Date(today.getTime() + 45 * 24 * 60 * 60 * 1000); // +45 days
    }
