...
type BindEnterpriseOperationFormProps = {
  enterprise: Partial<Enterprise>;
  setFormController: (finalizar?: boolean) => void;
  setEnterprise: Dispatch<SetStateAction<Partial<Enterprise | null>>>;
};

export default function BindEnterpriseOperationForm({
  setFormController,
  enterprise,
  setEnterprise,
}: Readonly<BindEnterpriseOperationFormProps>) {
  const {
    register,
    handleSubmit,
  ...
  const onSubmit = (data: Inputs) => {
    ...
    setEnterprise({ ...enterprise, ...enterpriseData });
    console.log(enterprise);
    setFormController();
  };

  useEffect(() => {
    if (enterprise.idExterno) {
      setFormController();
    }
  }, [enterprise]);
